...
const CENTUM_SCENES = [
  { id: "hero", name: "Centum Electronics Case Study", label: "01 / 21", startFrame: 0, endFrame: 0 },
  { id: "founding", name: "The Founding Story", label: "02 / 21", startFrame: 1, endFrame: 1 },
  { id: "business-understanding-1", name: "Understanding the Business - Financials", label: "03 / 21", startFrame: 2, endFrame: 2 },
  { id: "business-understanding-2", name: "Understanding the Business - Insights", label: "04 / 21", startFrame: 3, endFrame: 3 },
  { id: "standalone", name: "Standalone Business - P&L", label: "05 / 21", startFrame: 4, endFrame: 4 },
  ...
  { id: "orderbook", name: "Order Book Backlog", label: "15 / 21", startFrame: 14, endFrame: 14 },
  { id: "technology", name: "Exploded Payload Tech", label: "16 / 21", startFrame: 15, endFrame: 15 },
  { id: "space", name: "Orbital Integration", label: "17 / 21", startFrame: 16, endFrame: 16 },
  { id: "defence", name: "Defense Systems", label: "18 / 21", startFrame: 17, endFrame: 17 },
  { id: "efficiency", name: "Capital Metrics", label: "19 / 21", startFrame: 18, endFrame: 18 },
  { id: "valuation", name: "Valuation Explorer", label: "20 / 21", startFrame: 19, endFrame: 19 },
  { id: "thesis", name: "Investment Climax", label: "21 / 21", startFrame: 20, endFrame: 20 },
];
...
// Table 3 — the standalone P&L. Rendered identically (same width via its grid
// column, same internals) on every standalone frame so its dimensions match.
function StandalonePLCard() {
  const rows: FinRow[] = [
    { label: "Operational revenue", v: ["500.6", "632.8", "776.1", "973.1"] },
    { label: "Revenue YoY (%)", v: ["–", "+26.4", "+22.6", "+25.4"], sub: true },
    { label: "EBITDA", v: ["53.6", "78.7", "94.1", "120.9"] },
    { label: "EBITDA YoY (%)", v: ["–", "+46.8", "+19.6", "+28.5"], sub: true },
    { label: "EBITDA margin (%)", v: ["10.7", "12.4", "12.1", "12.4"], sub: true },
    { label: "Other income (incl. finance income)", v: ["4.7", "6.7", "6.6", "15.8"] },
    { label: "Depreciation", v: ["16.2", "18.4", "19.5", "19.5"] },
    { label: "Finance cost", v: ["15.7", "18.0", "19.5", "16.8"] },
    { label: "Profit before tax, pre-exceptional", v: ["26.4", "49.0", "61.7", "100.4"] },
    { label: "Exceptional items", v: ["–", "–", "–", "(203.3)"], sub: true },
    { label: "Reported profit before tax", v: ["26.4", "49.0", "61.7", "(102.9)"] },
    { label: "Tax", v: ["7.0", "12.7", "16.1", "14.2"] },
    { label: "Reported PAT", v: ["19.4", "36.3", "45.6", "(117.1)"], emphasis: true },
    { label: "Diluted EPS (Rs)", v: ["14.91", "27.82", "34.84", "(79.32)"], emphasis: true },
  ];

  return (
    <div className="w-full bg-[#0a0a14]/60 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto text-left">
      <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-1.5">
        Table 3. Standalone P&amp;L, last 4 years (Rs crore)
      </h3>
      <p className="text-[11px] text-white/50 font-light leading-snug mb-4">
        The standalone entity is the core India business, and the cleaner read on operating performance.
      </p>
      <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
        <thead>
          <tr className="border-b border-white/10 text-white font-bold">
        ...
        <FinTableRows rows={rows} />
      </table>
      <span className="block mt-4 font-mono text-[9px] text-white/40">
        FY26 reported profit reflects a one-time exceptional charge from the overseas restructuring.
      </span>
    </div>
  );
}
...
// 1.6. Section 1.6: Standalone Business (P&L)
function SceneStandaloneBusiness() {
  return <StandaloneFrameShell label="01.6. Standalone Entity" />;
}

// 1.7. Section 1.7: Standalone Business (Revenue by Segment)
function SceneStandaloneSegments() {
  const mix: FinRow[] = [
    { label: "EMS", v: ["70.7", "78.8", "74.0", "71.5"] },
    { label: "BTS", v: ["29.3", "21.2", "26.0", "28.5"] },
  ];
  const absolute: FinRow[] = [
    { label: "EMS revenue", v: ["353.8", "498.5", "574.3", "696.1"] },
    { label: "EMS YoY (%)", v: ["–", "+40.9", "+15.2", "+21.2"], sub: true },
    { label: "BTS revenue", v: ["146.7", "134.3", "201.8", "277.0"] },
    { label: "BTS YoY (%)", v: ["–", "(8.5)", "+50.3", "+37.3"], sub: true },
    { label: "Total revenue", v: ["500.5", "632.8", "776.1", "973.1"], emphasis: true },
  ];

  return (
    <StandaloneFrameShell label="01.7. Revenue By Segment">
      <div className="bg-[#0a0a14]/40 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto">
        <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-3">
          Table 4. Standalone revenue mix by segment (%)
        </h3>
        <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
          <thead>
            <tr className="border-b border-white/10 text-white font-bold">
          ...
          <FinTableRows rows={mix} />
        </table>
      </div>

      <div className="bg-[#0a0a14]/40 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto">
        <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-3 leading-snug">
          Table 5. Standalone revenue by segment, absolute (Rs crore) with YoY growth
        </h3>
        <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
          <thead>
            <tr className="border-b border-white/10 text-white font-bold">
...
// 1.75. Section 1.75: Standalone Order Book by Segment
function SceneStandaloneOrderBook() {
  const mix: FinRow[] = [
    { label: "EMS", v: ["39.7", "38.3", "50.4", "52.5"] },
    { label: "BTS", v: ["60.3", "61.7", "49.6", "47.5"] },
  ];
  const absolute: FinRow[] = [
    { label: "EMS order book", v: ["376.5", "427.8", "675.3", "864.2"] },
    { label: "EMS YoY (%)", v: ["–", "+13.6", "+57.9", "+28.0"], sub: true },
    { label: "BTS order book", v: ["571.5", "690.0", "665.7", "780.6"] },
    { label: "BTS YoY (%)", v: ["–", "+20.7", "(3.5)", "+17.3"], sub: true },
    { label: "Total order book", v: ["948.0", "1,117.8", "1,341.0", "1,644.8"], emphasis: true },
    { label: "Total YoY (%)", v: ["–", "+17.9", "+20.0", "+22.7"], sub: true },
  ];

  return (
    <StandaloneFrameShell label="01.9. Order Book By Segment">
      <div className="bg-[#0a0a14]/40 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto">
        <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-3">
          Table 6. Standalone order book mix by segment (%)
        </h3>
        <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
          <thead>
            <tr className="border-b border-white/10 text-white font-bold">
      ...
      <div className="bg-[#0a0a14]/40 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto">
        <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-3 leading-snug">
          Table 7. Standalone order book by segment, absolute (Rs crore, period-end) with YoY growth
        </h3>
        <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
          <thead>
            <tr className="border-b border-white/10 text-white font-bold">
...
// 1.11. Section 1.11: Standalone Valuations
function SceneStandaloneValuations() {
  const rowsTable10: FinRow[] = [
    { label: "18%", v: ["157", "169", "180", "186", "199", "212"] },
    { label: "20%", v: ["160", "171", "183", "192", "206", "220"] },
    { label: "25%", v: ["167", "179", "191", "208", "223", "238"] },
  ];
  const rowsTable11: FinRow[] = [
    { label: "18%", v: ["33.1", "30.9", "29.0"] },
    { label: "20%", v: ["32.6", "30.4", "28.5"] },
    { label: "25%", v: ["31.3", "29.2", "27.4"] },
  ];
  const rowsTable12: FinRow[] = [
    { label: "18%", v: ["28.1", "26.2", "24.6"] },
    { label: "20%", v: ["27.1", "25.3", "23.7"] },
    { label: "25%", v: ["25.0", "23.3", "21.9"] },
  ];

  return (
    <div className="max-w-6xl w-full px-4 z-10 flex flex-col h-full py-8 overflow-y-auto no-scrollbar justify-center">
      <div className="my-auto w-full flex flex-col gap-6">
        <div className="text-center w-full flex flex-col items-center">
          <SceneLabel>01.11. Valuations</SceneLabel>
          <h2 className="text-2xl sm:text-4xl md:text-5xl font-extrabold text-white tracking-tight leading-tight uppercase mb-3">
            Valuations
          </h2>
          <p className="text-xs sm:text-sm text-white/50 max-w-2xl mx-auto">
            Sensitivity analysis of implied forward EBITDA and valuation multiples based on revenue growth and margin targets.
          </p>
        </div>

        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 w-full max-w-6xl mx-auto items-stretch pointer-events-auto">
          {/* Table 10 */}
          <div className="xl:col-span-6 bg-[#0a0a14]/40 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto flex flex-col justify-between">
            <div>
              <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-3 leading-snug">
                Table 10. Implied forward EBITDA (Rs crore)
              </h3>
              <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
                <thead>
                  <tr className="border-b border-white/10 text-white font-bold text-center">
                    <th className="py-2 pr-2 font-sans font-semibold text-left" rowSpan={2}>Revenue growth (CAGR)</th>
                    <th className="py-1 px-2 border-b border-white/5 border-l border-white/5 text-center font-bold text-white" colSpan={3}>FY27 EBITDA margin</th>
                    <th className="py-1 px-2 border-b border-white/5 border-l border-white/5 text-center font-bold text-white" colSpan={3}>FY28 EBITDA margin</th>
                  </tr>
                  <tr className="border-b border-white/10 text-white font-bold">
                    <th className="py-1 px-2 text-right border-l border-white/5">14%</th>
                    <th className="py-1 px-2 text-right">15%</th>
                    <th className="py-1 px-2 text-right">16%</th>
                    <th className="py-1 px-2 text-right border-l border-white/5">14%</th>
                    <th className="py-1 px-2 text-right">15%</th>
                    <th className="py-1 px-2 text-right">16%</th>
                  </tr>
                </thead>
                <FinTableRows rows={rowsTable10} />
              </table>
            </div>
            <div className="mt-4 font-mono text-[9px] text-white/40 leading-relaxed font-light">
              Columns: EBITDA margin, grouped by forecast year.
            </div>
          </div>

          {/* Table 11 */}
          <div className="xl:col-span-3 bg-[#0a0a14]/40 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto flex flex-col justify-between">
            <div>
              <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-3 leading-snug">
                Table 11. Forward EV/EBITDA (x), FY27, EV held at Rs 5,210 crore
              </h3>
              <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
                <thead>
                  <tr className="border-b border-white/10 text-white font-bold text-center">
                    <th className="py-2 pr-2 font-sans font-semibold text-left" rowSpan={2}>Revenue growth (CAGR)</th>
                    <th className="py-1 px-2 border-b border-white/5 border-l border-white/5 text-center font-bold text-white" colSpan={3}>EBITDA margin</th>
                  </tr>
                  <tr className="border-b border-white/10 text-white font-bold">
                    <th className="py-1 px-2 text-right border-l border-white/5">14%</th>
                    <th className="py-1 px-2 text-right">15%</th>
          ...
          <div className="xl:col-span-3 bg-[#0a0a14]/40 border border-white/5 rounded-xl p-5 backdrop-blur-sm overflow-x-auto flex flex-col justify-between">
            <div>
              <h3 className="font-mono text-[10px] text-[#FFB800] uppercase tracking-wider mb-3 leading-snug">
                Table 12. Forward EV/EBITDA (x), FY28, EV held at Rs 5,210 crore
              </h3>
              <table className="w-full text-left font-mono text-[11px] text-white/80 border-collapse">
                <thead>
                  <tr className="border-b border-white/10 text-white font-bold text-center">
...
// 1.8. Section 1.8: Segment Capabilities (what sits inside EMS and BTS)
function SceneSegmentCapabilities() {
  const segments = [
    {
      id: "bts",
      subtitle: "Build-to-Specification · Design-led",
      desc: "Centum does the engineering and then manufactures, so it owns the design and the intellectual property — mission-critical subsystems where, after years of qualification, the incumbent is effectively irreplaceable.",
      groups: [
        { k: "Space", v: "Earth-observation satellite subsystems, electronic-warfare payloads" },
        { k: "Defence", v: "Missile control & guidance, radar subsystems, tank electronics" },
        { k: "Railways", v: "Passenger-information systems" },
      ],
      note: "Each programme is qualified over years with customers like ISRO & DRDO.",
      margin: "~20% EBITDA margin",
    },
    {
      id: "ems",
      subtitle: "Electronic Manufacturing Services · Build-to-print",
      desc: "The customer owns the design and Centum manufactures it at high reliability and high complexity, plus aftermarket and full lifecycle support.",
      groups: [
        { k: "Scope", v: "PCB assemblies, complex box builds, Line Replaceable Units (LRUs), full system integration" },
        { k: "Verticals", v: "Industrial & medical, electric mobility, defence & aerospace" },
        { k: "Anchor", v: "Fast-growing semiconductor-equipment vertical" },
      ],
      note: "“This is build-to-print work, so margins run nearer 10–12%.” — Nikhil Mallavarapu, Joint MD",
      margin: "10–12% EBITDA margin",
    },
  ];

  return (
    <div className="max-w-5xl w-full px-4 z-10 flex flex-col h-full py-8 overflow-y-auto no-scrollbar">
      <div className="my-auto w-full flex flex-col items-center text-center">
        <SceneHeading sub="01.8. Segment Capabilities" main="What Sits Inside EMS & BTS" />
        <p className="text-xs sm:text-sm text-white/50 mb-8 max-w-2xl mx-auto">
          Centum runs two businesses on different rungs of the same electronics value chain, which is why their economics differ.
        </p>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl w-full pointer-events-auto">
          {segments.map((s) => (
            <div
              key={s.id}
              className="bg-[#0a0a14]/80 border border-white/10 hover:border-[#FFB800]/40 rounded-2xl p-6 text-left backdrop-blur-md shadow-2xl transition-all duration-300 flex flex-col"
            >
              <div className="flex justify-between items-baseline gap-3 mb-3 border-b border-white/5 pb-2">
                <h3 className="text-lg font-bold text-white uppercase tracking-wider shrink-0">
                  {s.id.toUpperCase()}
                </h3>
                <span className="font-mono text-[9px] text-[#FFB800] uppercase tracking-wider font-bold text-right leading-snug">
                  {s.subtitle}
                </span>
              </div>

              <p className="text-xs text-white/60 leading-relaxed font-light mb-4">{s.desc}</p>

              <div className="space-y-2.5 mb-4">
                {s.groups.map((g) => (
                  <div key={g.k} className="flex gap-3">
                    <span className="font-mono text-[9px] text-[#FFB800] uppercase tracking-wider font-bold w-14 shrink-0 pt-0.5">
                      {g.k}
                    </span>
                    <span className="text-[11px] text-white/70 font-light leading-snug">{g.v}</span>
                  </div>
                ))}
              </div>

              <p className="text-[10px] text-white/45 italic font-light leading-snug mb-4">{s.note}</p>
...
export default function CentumElectronicsPage() {
  ...
  // Scroll transforms, one per frame. Breakpoints are derived from TOTAL_FRAMES
  // so inserting/removing a frame keeps every scene's fade window aligned.
  const SEG = 1 / TOTAL_FRAMES;
  const FADE_IN = 0.25 * SEG; // lead-in before the frame's slot
  const FADE_HOLD = 0.35 * SEG; // tail trimmed from the plateau before fade-out
  // [fade-in start, hold start, hold end, fade-out end] for frame index i.
  ...
  const sNew1Opacity = useTransform(progress, fr(2), FADE);
  const sNew1Y = useTransform(progress, fr(2), RISE);

  const sNew2Opacity = useTransform(progress, fr(3), FADE);
  const sNew2Y = useTransform(progress, fr(3), RISE);

  const sStandaloneOpacity = useTransform(progress, fr(4), FADE);
  const sStandaloneY = useTransform(progress, fr(4), RISE);

  const sStandalone2Opacity = useTransform(progress, fr(5), FADE);
  const sStandalone2Y = useTransform(progress, fr(5), RISE);

  const sSegCapOpacity = useTransform(progress, fr(6), FADE);
  const sSegCapY = useTransform(progress, fr(6), RISE);
  ...
  const s4Opacity = useTransform(progress, fr(12), FADE);
  const s4Y = useTransform(progress, fr(12), RISE);

  const s5Opacity = useTransform(progress, fr(13), FADE);
  const s5Y = useTransform(progress, fr(13), RISE);

  const s6Opacity = useTransform(progress, fr(14), FADE);
  const s6Y = useTransform(progress, fr(14), RISE);

  const s7Opacity = useTransform(progress, fr(15), FADE);
  const s7Y = useTransform(progress, fr(15), RISE);

  const s8Opacity = useTransform(progress, fr(16), FADE);
  const s8Y = useTransform(progress, fr(16), RISE);

  const s9Opacity = useTransform(progress, fr(17), FADE);
  const s9Y = useTransform(progress, fr(17), RISE);

  const s10Opacity = useTransform(progress, fr(18), FADE);
  const s10Y = useTransform(progress, fr(18), RISE);

  const s11Opacity = useTransform(progress, fr(19), FADE);
  const s11Y = useTransform(progress, fr(19), RISE);

  const s12Opacity = useTransform(progress, [20 * SEG - FADE_IN, 20 * SEG, 1.0], [0, 1, 1]);
  const s12Y = useTransform(progress, [20 * SEG - FADE_IN, 20 * SEG, 1.0], [24, 0, 0]);
  ...
  return (
    <div className="min-h-screen bg-[#030308] text-white font-sans selection:bg-[#FFB800] selection:text-[#030308] relative overflow-x-hidden">
      ...
      <div ref={containerRef} className="relative w-full h-[2000vh] bg-[#030308]">
        <div className="sticky top-0 w-full h-[100dvh] overflow-hidden flex items-center justify-center bg-[#030308] z-10">
          ...
          {presentationActive && (
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className={`${[0, 20].includes(currentFrameIndex) ? "absolute inset-0 w-full h-full z-10 pointer-events-auto" : SLIDE_BASE} text-center pointer-events-auto h-full`}
              >
                ...
                {currentFrameIndex === 4 && <SceneStandaloneBusiness />}
                ...
                {currentFrameIndex === 14 && <SceneOrderBook active={true} />}
                {currentFrameIndex === 15 && <SceneTechnology />}
                {currentFrameIndex === 16 && <SceneSpaceBusiness />}
                {currentFrameIndex === 17 && <SceneDefenceBusiness active={true} />}
                {currentFrameIndex === 18 && <SceneCapitalEfficiency />}
                {currentFrameIndex === 19 && <SceneValuation />}
                {currentFrameIndex === 20 && <SceneThesis presentationActive={true} />}
              </motion.div>
            </AnimatePresence>
          )}
          ...
          {!presentationActive && (
            <>
              ...
              <motion.div
                style={{ opacity: sStandaloneOpacity, y: sStandaloneY }}
                className={`${SLIDE_BASE} text-center ${
                  currentFrameIndex === 4 ? "pointer-events-auto" : "pointer-events-none"
                }`}
              >
                <SceneStandaloneBusiness />
              </motion.div>

              <motion.div
                style={{ opacity: sStandalone2Opacity, y: sStandalone2Y }}
                className={`${SLIDE_BASE} text-center ${
                  currentFrameIndex === 5 ? "pointer-events-auto" : "pointer-events-none"
                }`}
              >
                <SceneStandaloneSegments />
              </motion.div>

              <motion.div
                style={{ opacity: sSegCapOpacity, y: sSegCapY }}
                className={`${SLIDE_BASE} text-center ${
                  currentFrameIndex === 6 ? "pointer-events-auto" : "pointer-events-none"
                }`}
              >
                <SceneSegmentCapabilities />
              </motion.div>

              <motion.div
                style={{ opacity: sOrderSegOpacity, y: sOrderSegY }}
                className={`${SLIDE_BASE} text-center ${
                  currentFrameIndex === 7 ? "pointer-events-auto" : "pointer-events-none"
                }`}
              >
                <SceneStandaloneOrderBook />
              </motion.div>
              ...
              >
                <SceneFinancials active={currentFrameIndex === 12} />
              </motion.div>
              ...
              >
                <SceneOrderBook active={currentFrameIndex === 14} />
              </motion.div>
              ...
              >
                <SceneDefenceBusiness active={currentFrameIndex === 17} />
              </motion.div>
              ...
              <motion.div
                ...
                className={`absolute inset-0 w-full h-full z-10 ${
                  currentFrameIndex === 19 ? "pointer-events-auto" : "pointer-events-none"
                }`}
              >
                <SceneValuation />
              </motion.div>
